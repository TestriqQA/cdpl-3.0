...
> **Total entries:** 205 (BLG-001 → BLG-205)
> **Severity:** 19 P0 · 76 P1 · 64 P2 · 46 P3
> **Generated:** Cycle 1 audit, 2026-05-19 — extended 2026-05-28 (Phase 10c BLG-156 investigation: +BLG-200 P1 + BLG-201 P3; Phase 10d BLG-189 investigation: +BLG-202 P3 + BLG-203 P3 + BLG-204 P3; non-defensible-stats audit: +BLG-205 P0)
...
| BLG-202 | `cinutedigital.net` domain investigation — domain doesn't resolve but 3 backlinks reference it in GSC. Defensive WHOIS lookup: expired CDPL registration to renew + 301 to `.com`? typo-domain to ignore? third-party speculative? Brand-protection scope — **INVESTIGATED 2026-05-28** (Phase 10e doc `10e-blg-202-cinutedigital-net-whois.md` — Verisign RDAP pull: domain is **actively owned by a third party** via Hostinger registrar, registered 2018-01-17 (predates CDPL by 2 years), renewed 2026-01-06, parked on `dns-parking.com` nameservers. 5 forward options laid out; **recommendation**: Option 2 (free defensive monitoring) immediately + Option 3 (purchase offer to Hostinger broker, ~$200-1500 budget) if convenient. Not actionable as engineering — handoff to brand-protection owner). | P3 | 10d | external | Q |
| BLG-203 | Anchor-text outreach template — 4 of top 5 external anchors are URL-shaped or pure brand; only "certification preparation" is a real keyword anchor. When participating in directory submissions / award submissions / review platforms, pre-write the requested keyword anchor ("Software Testing Course in Mumbai by Cinute Digital", not "Cinute Digital") — **DRAFT READY 2026-05-28** (`docs/seo-audit/anchor-text-outreach-template.md` on audit branch — Tier 1 (8 commercial anchors) / Tier 2 (8 supporting) / Tier 3 (3 fall-back brand) priority list, per-channel templates for directory submissions / awards / podcasts / reviews / partnerships / guest posts, 3-step pre-paste checklist, and impact-tracking procedure). **Owner-side work pending:** content/marketing/founder team uses the template on each future outreach. | P3 | 10d | content | Q |
| BLG-204 | Internal-link audit on `/courses/software-testing-course/python-course` — internally linked 953 times, far more than any other course detail page. Either intentional anchor-course or hard-coded sibling-link in some component. Verify + redistribute if accidental — **CLOSED-AS-INTENDED 2026-05-28** (verified: 290 module-links in `courseData.ts` = 181 DS/ML/AI city-page modules + ~96 SWT city-page modules + Footer/Menu + city-courses module enumeration = ~953 total. Each of the 7 DS/ML/AI courses appears exactly 181× = 181 DS/ML/AI city-pages × 7 modules-per-page, templated structure. python-course is the ONLY course present in BOTH the SWT module list AND the DS/ML/AI module list — testers learn python for automation; data scientists need it foundationally — so it accumulates 3× the internal-link count by design. Editorially correct cross-family linking, not a bug). | P3 | 10d | — | Q |
| BLG-205 | Visible-UI non-defensible-stats cleanup — site-wide grep (`docs/seo-audit/non-defensible-stats-audit.md`, 2026-05-28) found **66 occurrences** of forbidden claims STILL in visible JSX / page metadata / FAQ data: 28 "100% placement [support/assistance]", 18 "15+ years industry experience" hero badges, 6 "5,000+ placed/students/alumni", 5 "50+ hiring partners" (one inflated to 500+ — internally inconsistent), 9 "guaranteed (job/interview/placement)" + 1 "95% placement rate". JSON-LD layer of this same issue (22 occurrences in `schema-generators.ts`) shipped today via `fix/blg-059-schema-generators-cleanup` (`bdfb27e`). The visible UI cleanup needs brand-approved replacement copy — content/marketing team scope. Doc lists every file:line with the offending phrase + recommended substitution pattern. Severity bumped from "spawned chip" (homepage hero) to P0 because (a) "100% placement" is legally risky in many jurisdictions, (b) inconsistent stats (50 vs 500 partners) damage credibility, (c) `_decisions.md` explicitly forbids these claims. **SHIPPED 2026-06-01** via `fix/blg-205-visible-ui-stats-cleanup` (191 files, 5,231 lines reframed) — scope expanded from the documented 66 to a full site-wide sweep including the `src/types/courseData.ts` city-page data (≈4,795 of the changed lines) and metadata keyword arrays. Guarantees → assistance/support; "100%/N% placement/success rate" → support framing or dropped; "15+/14+ years" collective claims → "Industry-Led Training"; student/alumni/graduate/partner counts → "4.9/5 across 425 verified reviews" / "hiring partner network" / removed; "best / #1 / top-rated / Top 1%" superlatives → neutral wording. Only the defensible 4.9 / 425-reviews signal retained. KEPT (intentional, legally safe): FAQ questions whose answers explicitly disclaim guarantees, money-back guarantees, named-instructor bios, salary-by-experience tiers, "best practices" technical term, analytics `source=` strings. Typecheck clean. Merged `develop`→`main` (`43c394d`); **Vercel production build PASSED (4m 16s) — LIVE in production.** | **P0** | 2026-06-01 | content | ✅ |
