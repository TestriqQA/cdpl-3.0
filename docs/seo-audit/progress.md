...
> **Updated:** 2026-05-22 — **CYCLE 2 + POST-CYCLE-2 WORK LIVE** ✅ — all 42 `fix/*` branches deployed. Post-Cycle-2 (all live, `origin/main` @ `bc6dfc8`): Q5 (founder `sameAs`); Q7 / BLG-139/140/146 draft-mode preview (Presentation tab + `<VisualEditing/>` working in `/cms`); draft-preview rolled out to all blog/jobs pages; **BLG-122** `/llms-full.txt`; **BLG-035** clean `/jobs/live-jobs/[jobId]` route + 301 from legacy `?jobId=`; **BLG-039** category slug-rename redirects via `previousSlugs`.
