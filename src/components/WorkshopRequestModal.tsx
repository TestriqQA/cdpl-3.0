...
const WorkshopRequestModal = ({
    ...
    return ReactDOM.createPortal(
        <>
            ...
            {/* CSS-based Modal */}
            {(isOpen || isVisible) && (
                <div className={`fixed inset-0 z-[10000] overflow-y-auto duration-300 transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
                    {/* Backdrop */}
                    <div
                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        <div
                            className={`relative w-full max-w-lg overflow-hidden rounded-2xl bg-white text-left shadow-2xl ring-1 ring-gray-900/5 transition-all duration-300 transform ${isOpen ? 'scale-100 translate-y-0 opacity-100' : 'scale-95 translate-y-4 opacity-0'
                                }`}
