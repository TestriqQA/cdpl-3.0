        ...
        faqs: [
            {
                ...
                answer: "We provide comprehensive mock assessments for Software Testing, Databases, Cloud Computing, Automation, Security, and Web Development."
            },
            {
                question: "Are the mock test results instant?",
                answer: "Yes, our platform provides real-time scoring and instant feedback upon completion of the assessment."
            },
            {
                question: "Is there a cost for the mock assessments?",
                answer: "Many of our basic assessments are free to access, providing instant results for students and professionals."
            }
        ],
        howToSteps: [
            { name: "Domain Selection", text: "Choose your technical domain from Software Testing, Cloud, Security, or Development." },
            { name: "Registration", text: "Provide your basic details to access the premium assessment portal." },
            { name: "Assessment", text: "Complete the industry-standard simulation test in a timed environment." },
            { name: "Results", text: "Receive instant certification and real-time score analysis upon completion." }
        ],
        categories: [
            "Software Testing",
            "Database Management",
    ...
    return (
        <div className="min-h-screen bg-white text-slate-800 font-sans selection:bg-brand-100 selection:text-brand-900">
            {mockSchemas.map((schema, index) => (
                <JsonLd key={`mock-schema-${index}`} id={`mock-schema-${index}`} schema={schema} />
            ))}
